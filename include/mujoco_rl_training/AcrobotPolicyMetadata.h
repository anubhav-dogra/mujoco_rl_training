...
struct AcrobotPolicyMetadata {
    AcrobotEnvConfig config;
...
namespace acrobot_metadata_detail {
...
inline void save_acrobot_policy_metadata(const std::string& metadata_path, const std::string& policy_path,
                                         const AcrobotEnvConfig& config, double best_return, int num_iterations,
                                         int episodes_per_evaluation, double noise_std_dev) {
    ...
    output << "control_name=" << config.control_name << '\n';
    acrobot_metadata_detail::write_vector(output, "joint_names", config.joint_names);
    acrobot_metadata_detail::write_vector(output, "target_angles", config.target_angles);
    acrobot_metadata_detail::write_vector(output, "angle_cost_weights", config.angle_cost_weights);
    acrobot_metadata_detail::write_vector(output, "target_link_angles", config.target_link_angles);
    acrobot_metadata_detail::write_vector(output, "link_angle_cost_weights", config.link_angle_cost_weights);
    acrobot_metadata_detail::write_vector(output, "velocity_cost_weights", config.velocity_cost_weights);
    acrobot_metadata_detail::write_vector(output, "control_cost_weights", config.control_cost_weights);
    acrobot_metadata_detail::write_vector(output, "max_torques", config.max_torques);
}

inline std::optional<AcrobotPolicyMetadata> load_acrobot_policy_metadata(const std::string& metadata_path) {
    const auto metadata = acrobot_metadata_detail::load_key_values(metadata_path);
    if (metadata.empty()) {
        return std::nullopt;
    }
    ...
    loaded.config.control_name = value("control_name");
    loaded.config.joint_names = acrobot_metadata_detail::parse_string_vector(value("joint_names"), "joint_names");
    loaded.config.target_angles = acrobot_metadata_detail::parse_double_vector(value("target_angles"), "target_angles");
    loaded.config.angle_cost_weights =
        acrobot_metadata_detail::parse_double_vector(value("angle_cost_weights"), "angle_cost_weights");
    loaded.config.target_link_angles =
        acrobot_metadata_detail::parse_double_vector(value("target_link_angles"), "target_link_angles");
    loaded.config.link_angle_cost_weights =
        acrobot_metadata_detail::parse_double_vector(value("link_angle_cost_weights"), "link_angle_cost_weights");
    loaded.config.velocity_cost_weights =
        acrobot_metadata_detail::parse_double_vector(value("velocity_cost_weights"), "velocity_cost_weights");
    loaded.config.control_cost_weights =
        acrobot_metadata_detail::parse_double_vector(value("control_cost_weights"), "control_cost_weights");
    loaded.config.max_torques = acrobot_metadata_detail::parse_double_vector(value("max_torques"), "max_torques");
    return loaded;
}
